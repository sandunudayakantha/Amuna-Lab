...
export const generateInvoicePDF = (invoice, user, tests) => {
    ...
    doc.text(`Gender: ${user?.gender || 'Not specified'}`, 20, 90);
    
    // Safely handle age display
    const ageYears = user?.age?.years || 0;
    const ageMonths = user?.age?.months || 0;
    const ageText = ageYears > 0 || ageMonths > 0 
        ? `${ageYears} years ${ageMonths} months`
        : 'Not specified';
    doc.text(`Age: ${ageText}`, 20, 100);
    
    doc.text(`Phone: ${user?.phone || 'Not specified'}`, 20, 110);
    doc.text(`Email: ${user?.email || 'Not specified'}`, 20, 120);
    doc.text(`Address: ${user?.address || 'Not specified'}`, 20, 130);
    ...
    const paymentRows = [
        ['Payment Type:', invoice.paymentType || 'Not specified', ''],
        ['Total Amount:', 'Rs.', (invoice.amount || 0).toFixed(2)],
        ['Amount Paid:', 'Rs.', (invoice.payingAmount || 0).toFixed(2)],
        ['Due Amount:', 'Rs.', (invoice.dueAmount || 0).toFixed(2)],
        ['Payment Status:', invoice.paymentStatus || 'Not specified', '']
    ];
