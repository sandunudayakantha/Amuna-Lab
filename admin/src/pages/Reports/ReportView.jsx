...
const ReportView = () => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/testReports/${id}`);
  ...
  return (
    <div>
      ...
      <div id="report-content" className="p-6 report-container">
        <div className="bg-white p-8 rounded shadow-md a4-sheet mt-36">
        ...
        <div className="screen-only mt-4">
          <button 
            onClick={handlePrint}
            className="bg-blue-500 text-white px-4 py-2 rounded-2xl hover:bg-blue-600 screen-only"
          >
            Download PDF
          </button>
          <button 
            onClick={handlePrintReport}
            className="bg-green-500 text-white px-4 py-2 rounded-2xl hover:bg-green-600 screen-only ml-2"
          >
            Print Report
          </button>
        </div>
