...
const InvoiceManager = () => {
    ...
    const handlePrintInvoice = async (invoice) => {
        try {
            ...
            // Fetch user details
            const userResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/${invoice.userId._id || invoice.userId}`);
            if (!userResponse.ok) {
                throw new Error('Failed to fetch user details');
            }
            const userData = await userResponse.json();
            
            // Fetch test templates if not already included
            let testTemplates = invoice.testTemplates;
            if (!testTemplates || testTemplates.length === 0) {
                const testResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/test-templates/${invoice.testTemplateId}`);
                if (testResponse.ok) {
                    const testData = await testResponse.json();
                    testTemplates = [testData];
                }
            }
            
            // Generate PDF using the utility
            const pdf = generateInvoicePDF(invoice, userData, testTemplates);
    ...
    const handleDeleteInvoice = async () => {
        try {
            await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/invoices/${selectedInvoice._id}`);
