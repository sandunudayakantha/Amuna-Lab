...
const Dashboard = () => {
  ...
  const fetchExpirySummary = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/inventory/status/expiry-summary`);
    ...
    try {
      ...
      switch (reportType) {
        case 'inventory':
          ...
          break;
        case 'low-stock':
          endpoint = '/api/inventory/reports/low-stock';
          break;
        case 'expired':
          endpoint = '/api/inventory/reports/expired';
          break;
        case 'expiring':
          endpoint = '/api/inventory/reports/expiring';
          break;
        default:
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}${endpoint}?format=${reportFormat}`,
        { 
          responseType: 'blob',
          headers: {
            'Accept': reportFormat === 'excel' 
      ...
      link.href = url;
      link.setAttribute('download', `${reportType}_report_${new Date().toISOString().split('T')[0]}.${reportFormat}`);
